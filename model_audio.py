...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, memory, tgt_mask=None, memory_mask=None):
        x = x + self.attn(self.ln_1(x), self.ln_1(x), self.ln_1(x), attn_mask=tgt_mask)[0]
        if memory is not None:
            x = x + self.crossattn(self.ln_1(x), memory, memory, attn_mask=memory_mask)[0]
...
class AudioTransformer(nn.Module):
    ...
    def forward(self, encoder_idxs, decoder_idxs, targets=None, tgt_mask=None):
        ...
        # Encoder processing
        encoder_emb = self.transformer.wte(encoder_idxs)
        encoder_pos = self.transformer.wpe(torch.arange(0, encoder_idxs.size(1), device=encoder_idxs.device))
        encoder_input = self.transformer.drop(encoder_emb + encoder_pos)
    ...
    def generate(self, encoder_idx, decoder_idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for n in range(max_new_tokens):
            ...
            encoder_idx_cond = encoder_idx[:, n: n + self.config.block_size]
            decoder_idx_cond = decoder_idx if decoder_idx.size(1) <= self.config.block_size else decoder_idx[:, -self.config.block_size:]
